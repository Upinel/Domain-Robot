...
int main(int argc , char *argv[]) {
    FILE * fp = fopen("database/TLD_DATA","r");
    if (fp==NULL) {
    ...
    scanf("%s",DictFile);
    char DictFileAddr[256];
    strcpy(DictFileAddr, "./dict/");
    strcat(DictFileAddr, DictFile);
    FILE * fp_Dict = fopen(DictFileAddr,"r");
    if (fp_Dict==NULL) {
    ...
    while (1) {
        ...
        *DomainPrefix='\0';
        ...
        while ((c = fgetc(fp_Dict)) != '\n') {
            if (c==EOF) {
                printf("TASK FINISHED!\n\nTHANK YOU FOR USING DOMAINMEGABOT POWERED BY UPINEL (HTTP://UPINEL.COM). HAVE A GREAT DAY!\n");
                fp_Result=fopen(outressults,"a");
                fprintf(fp_Result,"\nTHANK YOU FOR USING DOMAIN MEGABOT POWERED BY UPINEL (HTTP://UPINEL.COM). HAVE A GREAT DAY!\n");
                fclose(fp_Result);
                fclose(fp_Dict);
                exit(0);
            }
